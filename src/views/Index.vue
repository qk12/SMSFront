      ...
      <div style="display: flex; justify-content: space-between">
        ...
        <div style="display: flex; flex-direction: row">
          <div style="margin-right: 20px">
            欢迎 {{ name }}
            <a-icon @click="logout" type="logout" style="cursor: pointer" />
          </div>
          当前学期 &nbsp;&nbsp;
    ...
    <a-layout-content style="flex: 1; background: #fff">
      <a-layout style="padding: 24px 0; background: #fff">
        ...
        >
          <a-table
            v-show="activeKey === '1'"
            :columns="openCoursesCoulmns"
            :dataSource="openCoursesData"
            :rowKey="(record) => record.kh + record.gh"
          ></a-table>
          <a-table
            v-show="activeKey === '2'"
            :columns="openCoursesCoulmns"
          ...
          <div v-show="activeKey === '4'">
            <a-table
              ...
              :rowKey="(record) => record.kh + record.zpcj"
            ></a-table>
          ...
          <div v-show="activeKey === '8'">
            <a-table
              :columns="reportCardTeacherColumns"
              :dataSource="reportCradTeacherData"
              bordered
              :rowKey="(record) => record.xh + record.kh"
            >
              <template
                v-for="col in ['xm', 'xh', 'kh', 'km', 'pscj', 'kscj', 'zpcj']"
                :slot="col"
          ...
          <a-table
            v-show="activeKey === '10'"
            :columns="openCoursesTeacherColumns"
            :dataSource="openCoursesTeacherData"
            :rowKey="(record) => record.kh"
          ></a-table>
        </a-layout-content>
      </a-layout>
    </a-layout-content>
...
export default {
  ...
  computed: {
    ...mapGetters(["role", "userInfo", "terms", "nowTerm"]),
    name() {
  ...
  methods: {
    ...
    handleSubmit(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          const { kh, gh } = values;
          api
            .postXuanKe({ kh, gh, xh: this.userInfo.xh, xq: this.nowTerm })
            .then((res) => {
    ...
    handleSubmitOpen(e) {
      ...
      this.formOpen.validateFields((err, values) => {
        if (!err) {
          ...
          api
            .postKaiKe({ kh, sksj, gh: this.userInfo.gh, xq: this.nowTerm })
            .then((res) => {
              if (res.data.res.affectedRows === 1) {
    ...
    initData() {
      ...
      if (
        this.nowTerm &&
        !judgeJSObjectIsNull(this.userInfo) &&
        this.userInfo.gh
      ) {
        this.loadCourseTeacherTable(this.userInfo.gh, this.nowTerm);
        this.loadReportCardTeacher(this.userInfo.gh, this.nowTerm);
